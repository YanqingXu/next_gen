...
void runPerformanceTest(const std::string& queueType, size_t numProducers, size_t numConsumers, 
                        size_t messagesPerProducer, size_t queueCapacity = 1024) {
    std::cout << "\nRunning performance test for " << queueType << " queue:" << std::endl;
    ...
    auto queue = createMessageQueue(queueType, queueCapacity);
...
void demonstrateQueueTypes() {
    ...
    auto defaultQueue = createMessageQueue("default");
    auto priorityQueue = createMessageQueue("priority");
    auto lockFreeQueue = createMessageQueue("lockfree");
    auto mpmcQueue = createMessageQueue("mpmc");
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        lockFreeQueue->push(std::make_unique<TestMessage>(i));
        mpmcQueue->push(std::make_unique<TestMessage>(i));
    }
    ...
    while (!lockFreeQueue->empty()) {
        auto message = lockFreeQueue->tryPop();
        if (message) {
            auto* testMessage = dynamic_cast<TestMessage*>(message.get());
            if (testMessage) {
                std::cout << "  " << testMessage->toString() << std::endl;
...
int main() {
    ...
    std::cout << "Message Queue Example Started" << std::endl;
    ...
    runPerformanceTest("default", 1, 1, 100000);
    runPerformanceTest("priority", 1, 1, 100000);
    runPerformanceTest("lockfree", 1, 1, 100000);
    runPerformanceTest("mpmc", 1, 1, 100000);
    
    // Test 2: Multiple producers, single consumer
    runPerformanceTest("default", 4, 1, 25000);
    runPerformanceTest("priority", 4, 1, 25000);
    runPerformanceTest("lockfree", 4, 1, 25000);
    runPerformanceTest("mpmc", 4, 1, 25000);
    
    // Test 3: Single producer, multiple consumers
    runPerformanceTest("default", 1, 4, 100000);
    runPerformanceTest("priority", 1, 4, 100000);
    runPerformanceTest("lockfree", 1, 4, 100000);
    runPerformanceTest("mpmc", 1, 4, 100000);
    
    // Test 4: Multiple producers, multiple consumers
    runPerformanceTest("default", 4, 4, 25000);
    runPerformanceTest("priority", 4, 4, 25000);
    runPerformanceTest("lockfree", 4, 4, 25000);
    runPerformanceTest("mpmc", 4, 4, 25000);
    
    std::cout << "\nMessage Queue Example Completed" << std::endl;
